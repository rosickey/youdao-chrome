(function() {
	var playVoice=function(){
		var text=window.gdxBubbleInstance.q.innerHTML;
		chrome.runtime.sendMessage({type: "voice",text:text}, function(response) {
			});
	};

        var youdaorecord=function(){
            var text=window.gdxBubbleInstance.q.innerHTML;
            chrome.runtime.sendMessage({type: "record",text:text}, function(response) {
                });
        };
    ...
    k = function(target, func) {
        return function() {
            return func.apply(target, arguments)
        }
    },
    p = function(dom) {// is input dom
        if (dom && dom.tagName) {
            var tagName = dom.tagName.toLowerCase();
            if ("input" == tagName || "textarea" == tagName) return true;
        }
        if (document.designMode && "on" == document.designMode.toLowerCase()) return true;
        for (; dom; dom = dom.parentNode) if (dom.isContentEditable) return true;
        return false;
    },
    q = /[0-9A-Za-z]/,
    A = function() {
        ...
        function(a) {
            ...
            var f_f = document.createElement("input");
            f_f.type = "button";
            f_f.value="Add";
            b.target = "_blank";
            ...
            this.ff = f_f.cloneNode(!1);
            
            this.i.appendChild(this.f);
            this.i.appendChild(this.ff);
            ...
            x("click", youdaorecord, this.ff);
            x("click", playVoice, this.l);//play voice
    ...
    d.A = null;
    d.v = null;
    var x = function(a, b, c) {// addEventListener(event,handler,target)
        document.addEventListener ? c.addEventListener(a, b, !1) : c.attachEvent("on" + a, b)
    },
    C = function(a, b, c) {// removeEventListener(event,handler,target)
    ...
    E = function(a) {
        ...
        ff = document.documentElement.offsetWidth + document.documentElement.scrollLeft,
    ...
    A.prototype.L = function(a) {// Bubble
        if (a.eventKey == this.m) {
            ...
            if (a.meaningObj) {
                ...
                //this.p.src = b.audio;//原本是通过这里返回的声音链接，在本页中播放
                this.l.className = "gtx-display-block";
                
                
                //this.f.href = b.moreUrl;
                this.f.href = "http://dict.youdao.com/search?q="+this.q.innerHTML;
                this.f.innerHTML = "更多 \u00bb";
            ...
            this.f.innerHTML = '谷歌搜索 "' + a.sanitizedQuery + '" \u00bb';
